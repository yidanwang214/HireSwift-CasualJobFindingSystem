...
const JobDescription = () => {
    ...
    const {
        tags, categoryId, status, ownerId, title, description, location, contact, salaryStart,
        salaryEnd, createdAt, updatedAt, id, category, ownerInfo, ownerRating, applicants
      } = job;
    return (
        <div id='jobDescription' className='flex flex-row mt-5 font-sans'>
            <div id='jobDetails' className='basis-2/3 mx-10'>
                <div id='title'  className='flex flex-row align-middle'>
                     <h1 id='jobtitle' className='text-2xl font-bold'>{title}</h1>
                </div>
                    ...
                    <div id='textInfo' className='basis-2/3 flex-1 flex flex-col justify-items-start'>
                        <div id='name' className='flex flex-row justify-between'>
                            ...
                            <div className='flex flex-row'>
                                <Rating name="half-rating-read" value={job.ownerRating} precision={0.5} readOnly />
                                {job.ownerRating}
                            </div>
                        </div> 
                        <div id='wage'>
                ...
                <div id='review' className='mt-10 text-justify'>
                    ...
                    {!reviews ? (
                        ...
                        reviews.map((item) => (
                            <div id={item.id} key={item.id} className='border-b border-b-zinc-300 mt-2'>
                                Rating: {item.rating}
            ...
            <div id='apply' className='basis-1/3 mx-10 rounded-md border-2 border-slate-400 h-3/4 items'>
                ...
                <CopyToClipboardModal open={isModalOpen} onClose={() => setIsModalOpen(false)} />
            </div>        
        </div>
    )
}
